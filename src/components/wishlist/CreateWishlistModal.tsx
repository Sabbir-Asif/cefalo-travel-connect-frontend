    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const wishlistData: CreateWishlist = {
                title: title.trim(),
                location_name: travelPlace.location_name,
                location_point: travelPlace.location_point,
                travel_date: new Date(travelDate),
                tags: tags.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0),
                note: note.trim() || undefined,
                cover_image: coverImage.trim() || travelPlace.cover_image,
                status,
                travel_place_id: travelPlace.id
            };
        ...
        <div className="modal modal-open">
            <div className="modal-box max-w-md">
                ...
                <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="font-medium">{travelPlace.name}</p>
                    <p className="text-sm text-gray-500">{travelPlace.location_name}</p>
                </div>

...
import { removeTransportFromTravelPlanAPI } from "../../../utils/api/travel-plan";
...
const TransportCard: React.FC<TransportCardProps> = ({ transport, onDelete }) => {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const status = await removeTransportFromTravelPlanAPI(transport.id);
      if (status === 204) {
